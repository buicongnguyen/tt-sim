...
const phases = {
  ...
  boot: {
    ...
    url: "https://github.com/tenstorrent/tt-metal/blob/50a82f835593512c4176546b4af68d7e91315a86/tt_metal/impl/device/firmware/risc_firmware_initializer.cpp#L1053-L1199",
  },
  dispatch: {
    number: "03",
    label: "CQ bring-up",
...
const objectLayers = [
  { number: "A", title: "Base RISC firmware", lifetime: "DEVICE SESSION", detail: "Separate BRISC, NCRISC and TRISC0/1/2 control-loop ELFs loaded before worker release.", source: "initialize_firmware()", url: "https://github.com/tenstorrent/tt-metal/blob/50a82f835593512c4176546b4af68d7e91315a86/tt_metal/impl/device/firmware/risc_firmware_initializer.cpp#L1053-L1199" },
...
const bootSteps = [
  ...
  { id: "B3", from: "Host", to: "Worker L1", title: "Initialize launch state", detail: "The host writes firmware tables, launch-ring fields and INIT/GO state.", sources: [{ label: "risc_firmware_initializer.cpp:1053–1123", url: "https://github.com/tenstorrent/tt-metal/blob/50a82f835593512c4176546b4af68d7e91315a86/tt_metal/impl/device/firmware/risc_firmware_initializer.cpp#L1053-L1123" }] },
  { id: "B4", from: "Host", to: "Worker L1", title: "Load every firmware image", detail: "Each ELF span is multicast to the HAL address assigned to its processor.", sources: [{ label: "risc_firmware_initializer.cpp:1143–1199", url: "https://github.com/tenstorrent/tt-metal/blob/50a82f835593512c4176546b4af68d7e91315a86/tt_metal/impl/device/firmware/risc_firmware_initializer.cpp#L1143-L1199" }] },
  { id: "B5", from: "Host", to: "BRISC", title: "Release the supervisor", detail: "On Wormhole and Blackhole, BRISC is the first worker RISC released by the host.", sources: [{ label: "risc_firmware_initializer.cpp:1495–1532", url: "https://github.com/tenstorrent/tt-metal/blob/50a82f835593512c4176546b4af68d7e91315a86/tt_metal/impl/device/firmware/risc_firmware_initializer.cpp#L1495-L1532" }] },
...
const sourceCards = [
  ...
  { id: "03", kind: "HOST BOOT", title: "Load every RISC image", detail: "Host initialization writes launch state and each firmware binary before release.", url: "https://github.com/tenstorrent/tt-metal/blob/50a82f835593512c4176546b4af68d7e91315a86/tt_metal/impl/device/firmware/risc_firmware_initializer.cpp#L1053-L1199" },
