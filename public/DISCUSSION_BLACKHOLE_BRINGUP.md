...
the operation ELF entry, CRT/data initialization, load image and ABI. Only after
...
| NCRISC firmware `R`, no operation `K` | control is between `kernel_lma` call and wrapper entry | load address, ELF entry, CRT/ABI |
...
flowchart TD
    ...
    W -->|NC:R / no NC:K| E[Entry, load, CRT or ABI]
...
**Answer:** preserve the host build, enable readback verification, inspect the
ELF and linker map, and map the failing PC to the same image.
...
```mermaid
flowchart TD
    H[Hash host ELF] --> R[Enable device kernel readback]
    R --> M{Bytes match?}
...
- [Kernel write/readback check](https://github.com/tenstorrent/tt-metal/blob/50a82f835593512c4176546b4af68d7e91315a86/tt_metal/llrt/llrt.cpp#L190-L250)
